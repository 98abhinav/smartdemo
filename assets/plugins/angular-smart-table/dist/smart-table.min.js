...
(function (ng, undefined) {
    ...
    ng.module('smart-table')
            .constant('stConfig', {
                pagination: {
                    template: 'shared/pagination.html',
                    itemsByPage: 10,
                    displayedPages: 5
                },
                search: {
    ...
    ng.module('smart-table')
            .directive('stRefresh', ['stConfig', '$parse', function (stConfig, $parse) {
                    return {
                        restrict: 'A',
                        require: '^stTable',
                        link: function (scope, element, attr, ctrl) {
                            function refresh() {
                                if (!angular.equals({}, ctrl.tableState().search)) {
                                    //ctrl.tableState().sort = {};
                                    ctrl.tableState().search = {};
                                    //ctrl.tableState().pagination.start = 0;
                                    ctrl.pipe();
                            ...
                            element.bind('click', function refreshClick() {
                                scope.$apply(refresh);
                            });
                        }
                    };
    ...
    ng.module('smart-table').directive('pageSelect', ['$timeout', function ($timeout) {
            return {
                restrict: 'E',
                template: '<input type="text" class="select-page" ng-model="inputPage" ng-model-options="{debounce: 500}" ng-change="selectPage(inputPage)">',
    ...
    ng.module('smart-table')
            .directive('stPaginationScroll', ['$timeout', function (timeout) {
                    return{
                        require: 'stTable',
                        link: function (scope, element, attr, ctrl) {
                            var itemByPage = ctrl.tableState().pagination.itemByPage;
                            ...
                            var container = angular.element('tbody');
